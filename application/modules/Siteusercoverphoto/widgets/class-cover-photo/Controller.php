 ...
 */
class Siteusercoverphoto_Widget_ClassCoverPhotoController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    if($this->view->sitememberEnabled) {
    ...
    $this->view->photo='';
    if(isset($event->class_cover)) {
			if (Engine_Api::_()->hasModuleBootstrap('advalbum')) {
				$this->view->photo = $photo = Engine_Api::_()->getItem('advalbum_photo', $event->class_cover);
			} else {
				$this->view->photo = $photo = Engine_Api::_()->getItem('album_photo', $event->class_cover);
			}
    }
    ...
    if( $can_edit && Engine_Api::_()->authorization()->isAllowed('siteusercoverphoto', $event, 'upload')) {
