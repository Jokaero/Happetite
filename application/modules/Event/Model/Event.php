...
class Event_Model_Event extends Core_Model_Item_Abstract
{
  ...
  public function getMemberCount($status = array())
  {
    if (null === $status or empty($status)) {
      return $this->membership()->getMemberCount(null);
    }
    ...
    if (!empty($status) and is_array($status)) {
      ...
      foreach ($status as $statusId) {
        $count += $this->membership()->getMemberCount(null, array('rsvp' => (int) $statusId));
      }
