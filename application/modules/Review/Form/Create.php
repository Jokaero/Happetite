...
class Review_Form_Create extends Engine_Form
{
  ...
  public function validateMember($value)
  {
    ...
    $friendonly = Engine_Api::_()->getApi('settings', 'core')->getSetting('review.friendonly', 0);
    
    if ($friendonly)
    {
      if (!$user->membership()->isMember($viewer, true)) {
        $this->toValues->getValidator('Engine_Validate_Callback')->setMessage('You can only write review for your friend. Please add this member as your friend first if you wish to write a review!');
        return false;
      }
    }
